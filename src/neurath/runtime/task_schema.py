...
TASKS = {
    "harness_bypass": ("harness", "bypass", "Read or switch off Neurath hook constraints for this worktree. On detecting a Neurath malfunction, call enabled=true autonomously without asking the user for permission; do not wait for tests or task bookkeeping. This applies to any harness malfunction. Notify the user, preserve unfinished work, and restore with enabled=false after verifying recovery. Omit enabled to read. Host security and user restrictions remain unchanged. This switch remains callable while hooks are bypassed.",
    ...
    "provider_run": ("provider-execution", "run", "Accept an authorized independent session using its exact model plan. mode=inherit preserves creator policy; explicitly approved target-native keeps target defaults, hooks and tool rules. Conflicting settings are rejected. Return durable run_id immediately; no task lifetime deadline. Verify actual model, policy, activation and ownership before assignment. States return through durable messages. Reuse key and identical request on uncertain retry. Acceptance is not completion. Never resumes a foreign live session. App observation is separate.",
        {"purpose": {**choice("task", "perspective", "worktree-worker", "user-session"), "default": "task"},
         "reason": text_field(2400, default=""),
    ...
    "provider_route": ("provider", "route", "Choose native children by default for work owned by this session. purpose=perspective requires another provider; purpose=worktree-worker requires the same provider and an isolated installed checkout. Both require a concrete reason, worktree and assignment. purpose=user-session requires expected user continuation. Return delegation_prepare, a planned provider_run, an app preparation route or a precise prerequisite. Execution settings retain native policy inheritance. Routing only: no creation, policy change or assignment has happened. IDs identify targets, never caller authority.",

...
> 이 문서는 리뷰 태세 전용 SSOT다. `/review-code`와 그 persona, `/triage-comments`가
> 명시적으로 읽어 오며, 리뷰 밖 구현 세션에는 주입되지 않는다.
...
새 코드는 charter §2 / `python-code.md` "최신 문법" / `type-discipline.md` 같은 **대원칙 SSOT**를 따른다. 같은 파일·도메인 안에 신구 스타일이 혼재하면 새 코드는 신스타일로 통일하고, 레거시 정리가 필요하면 `.agents/rules/behavioral.md` Gap Triage를 적용한다. 현재 변경의 acceptance나 merge safety를 깨는 정리는 지금 처리하고, 독립 work item일 때만 `/plan-issues` 또는 `/create-ticket`로 분기한다.
...
- 스펙 자체가 누락된 케이스 발견 시 지금 수정 제안(→ `behavioral.md` "질문"과
  `charter.md` "Code보다 제품 의도").
...
## 원칙 7 — DDD 표현력 / Model-first DDD drift

도메인 티켓이 adapter, usecase, policy, workflow, provenance, lifecycle 같은 행동 경계를
요구하는데 구현이 `AbstractValueObject`, `*Request`, `*Result`, `*Command` 같은 모델
shape 추가로만 수렴하면 의문. DDD (Domain-Driven Design)는 suffix/base class 매핑이
아니라 책임 소유권과 불변식 경계 표현이다.

**탐지 시그널:**

- 승인된 issue 또는 open ticket sequence가 행동 경계나 lifecycle을 요구하지만 diff가
  대부분 value object, request/result DTO (Data Transfer Object), enum, reference
  class만 추가한다.
- `domain/aggregates/<noun>/` package가 생겼지만 AggregateRoot, lifecycle owner,
  mutation method, domain event emission 중 어느 것도 없다.
- `*Request` 또는 `*Result`가 도메인 패키지 안에서 UseCase, policy evaluator,
  classifier, application service 책임을 대신 수행한다.
- `*Command`가 행동 의도를 표현하지만 대응하는 UseCase/application boundary 또는
  port contract가 없어 호출·검증·transaction boundary가 떠 있다.
- 리뷰나 하네스 응답이 "Request는 value object여야 한다" 같은 suffix/base class
  규칙으로만 흘러가고, 어떤 책임이 어디에 귀속되는지 묻지 않는다.
- 현재 PR이 후속 open ticket들의 기반 contract인데, 기존 code와 open ticket을 함께
  보지 않고 diff line만 보고 승인한다.

**판단 기준:**

- 이 신호가 architecture 방향, aggregate/port 경계, 후속 티켓 구현 방향을 바꾸면
  카테고리 4 "도메인 제네릭성 (Aggregate·Port 경계)"의 **Critical**이다.
- PR이 명시적으로 좁은 adapter DTO boundary만 만들고, 행동 경계가 후속 ticket에
  남아 있음을 문서화·검증했다면 Warning 또는 통과가 가능하다.
- review stage는 관련 open ticket summary와 이미 반영된 existing domain code를 같이
  읽어 모델-first drift 여부를 판단한다. diff만으로는 기반 contract의 방향성이
  맞는지 판단할 수 없다.
- 해결 질문은 "이 suffix가 어떤 DDD building block인가?"가 아니라 "이 행동과
  불변식의 owner가 UseCase, AggregateRoot, domain service, port, adapter 중 어디인가?"
  이어야 한다.

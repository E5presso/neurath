...
require_text "$autopilot_wave" '모순 C (auto-merge clean terminal 미흡수)' \
  'autopilot stuck detector must classify clean monitor_started as stuck'
require_text "$autopilot_wave" 'exact workflow skill state에 `monitor_started`' \
  'clean terminal stuck detection must start from exact workflow monitor_started state'
...
require_text "$autopilot_wave" '반환하지 말고 같은 turn에서 `gh pr merge --squash --delete-branch`와 cleanup까지' \
  'resume prompt must forbid returning after phase7_ready'

require_text "$phase6" '`monitor_event=terminal reason=mergeable-clean`은 Phase 6만의 terminal이므로' \
  'process-ticket Phase 6 must treat mergeable-clean as nonterminal for auto-merge'
require_text "$phase6" 'Phase 8 squash' \

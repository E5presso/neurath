...
Neurath의 installed CLI를 직접 호출할 때 current exact argv는 `graphify update .`와
`graphify cluster-only .`입니다. `/graphify . --update` 같은 slash-skill 입력을 shell
argv `graphify . --update`로 복사하지 않습니다.

# Graphify Full Build Runbook

This reference is loaded only for a fresh build from a path or URL, or a multi-path merge. It is
not required for an existing-graph query, path lookup, or node explanation. Start from the runtime
admission and interpreter decisions in `../SKILL.md`, then execute every applicable numbered step
below in order.
...
See `extraction-spec.md` for the exact subagent prompt (JSON schema, node-ID rules, confidence rubric, frontmatter, hyperedge, and vision rules). Load it only here, only when at least one chunk holds a doc, paper, or image; a pure-code corpus has skipped Part B and never reads it. Pass each subagent that prompt verbatim with FILE_LIST, CHUNK_NUM, TOTAL_CHUNKS, DEEP_MODE, and CHUNK_PATH substituted, and have it write the result to CHUNK_PATH.

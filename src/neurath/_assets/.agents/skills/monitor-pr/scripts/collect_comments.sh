...
# 사용법: REPO=example/project PR_NUMBER=23504 bash collect_comments.sh
#
# 채널:
...
#   in-place 갱신 감지에서 updatedAt이 증가한 id를 전달받아 본문이 바뀐 코멘트/리뷰를
#   재수집·재triage 대상으로 되돌린다.
set -euo pipefail

# shell tool spawns non-login shells that miss /opt/homebrew/bin.
...
ACTIONABLE_COMMENT_FILTER='
  def agent_signal:
    ((.body // "") | contains("<!-- claude-agent-reply"))
    or ((.body // "") | contains("<!-- ai-review verdict="))
    or ((.body // "") | test("^## AI Review Verdict:"))
    or ((.body // "") | test("^(## )?AI review 결과:"))
    or ((.body // "") | test("^(## )?AI 리뷰 결과:"))
    or ((.body // "") | test("^ai-review status (success|failure|pending) verified for "));

  def actionable_comment:
    (((.body // "") | test("<!-- [a-zA-Z0-9_-]*linkback -->")) | not)
    and (agent_signal | not)
...
# ── CH1: 인라인 리뷰 코멘트 ──
# 에이전트 자기 응답과 ai-review/status verification 신호 제외.
...
UNRESOLVED_THREADS=$(echo "$THREADS_RAW" | jq '
  [
...
' 2>/dev/null || echo "[]")
UNRESOLVED_THREADS_LEN=$(echo "$UNRESOLVED_THREADS" | jq 'length' 2>/dev/null || echo 0)

echo "CH1_COUNT=$CH1_LEN"
echo "CH2_COUNT=$CH2_LEN"
echo "CH3_COUNT=$CH3_LEN"
echo "UNRESOLVED_THREADS_COUNT=$UNRESOLVED_THREADS_LEN"
echo "TOTAL=$TOTAL"
...
if [ "$UNRESOLVED_THREADS_LEN" -gt 0 ]; then echo "UNRESOLVED_THREADS_DATA=$UNRESOLVED_THREADS"; fi

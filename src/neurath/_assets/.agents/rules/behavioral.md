...
현재 세션의 작업 위임은 티켓 전체를 포함해 native subagent를 기본으로 한다.
사용자가 직접 방문해 후속 작업을 이어갈 가능성이 있을 때만 사용자용 독립 세션을 만든다.
새로운 관점이 필요한 경우 다른 provider worker를 자율적으로 선택하고 그 이유를 기록한다.
실행 시간·worktree·새 런타임 자체는 사용자용 세션 생성 사유가 아니다. 앱 생성 도구의
명시적 사용자 요청 조건을 유지한다. Root는 구현 결과를 통합하고 별도의 새 리뷰 자식을
배정하며, 실제 모델·권한·활성화·소유권과 결과 회수까지 책임진다.
...
근거에 맞춰 사용합니다. 완료는 결과 설명과 근거를 한 번 기록합니다.
별도 워크플로 종결·추가 태스크 검토·TODO 표시 성공을 완료 조건으로 붙이지 않습니다.
...
확인해야 하면 프로젝트 설정의 검사 명령을 호스트의 명령 도구로 실행합니다.
관련 테스트부터 확인하고, 최종 소스에서 필요한 전체 검사를 한 번 수행합니다.

...
앱 프로젝트 소속이 필요한 새 Codex 작업은 provider_route의 앱 생성 경로를 먼저 확인한다.
앱 경로의 project_id는 list_projects의 앱 ID이며, native worker의 project_id와 혼용하지 않는다.
호스트 생성 도구의 명시적 새 작업·모델 선택 조건을 지키고 실제 설정을 관측한다.
session_status의 app_project는 현재 세션에 대한 앱의 로컬 소속 기록을 읽는 진단이다.
이 값은 권한·소유권·화면 표시의 증명이 아니며 미확인을 불가능으로 단정하지 않는다.
CLI 메타데이터 갱신을 앱 소속 완료로 표시하지 않는다. 앱 소속이 요구되면 앱이 소유한 생성·제어 경로를 사용한다.
루트는 Codex 또는 Claude일 수 있다. mode=inherit는 직전 발행자의 실제 권한과 제한을 승계한다.
각 provider의 고유 설정을 유지하도록 승인된 상호 위임은 mode=target-native를 명시한다.
target-native는 대상의 기존 기본 모드·훅·도구 규칙을 사용하고 실제 호스트에서 다시 확인한다.
inherit 실패를 이유로 target-native로 몰래 바꾸지 않는다. 두 모드는 별도의 실행 계획이다.
...
target-native를 선택한 계획과 실행에는 같은 mode를 사용한다. 대상 기본값을 관측하지
못했거나 지원하지 못하면 차단하고, 설정 파일을 변경하거나 허용 범위를 넓히지 않는다.
...
## 목표로 수렴하는 실행
현재 요청의 완료 조건과 남은 차이를 먼저 정하고, 각 행동이 그 차이를 줄이는지 확인한다.
상태 질문·재촉·불만은 기존 목표의 취소나 새 목표의 승인으로 해석하지 않는다.
새 조치는 원래 요구의 미충족 조건 또는 재현된 차단에 연결한다. 관련 없는 개선은 기록만 남긴다.
에이전트가 만든 검증·복구·최적화 목표는 교체 가능한 수단이며 사용자 목표와 동급의 의무가 아니다.
...
필요한 태스크 추가는 자율적으로 수행한다. 태스크 수를 줄이려고 미완료 요구를 누락하지 않는다.
현재 태스크를 종결하기 전에 발견한 필수 후속 작업이 원장에 있는지 확인한다. 이미 기록된 작업은
중복 등록하지 않고 완료 근거를 재사용한다. 실패 이력 이후에도 유효한 요구는 최신 성과와 대조하여
남은 부분만 후속 태스크로 등록하고 task_start로 이어간다. 명시적 취소와 답변만 요청한 턴은 존중한다.
후속 메시지 뒤 태스크를 추가할 때는 task_list의 기존 요구 또는 명시적인 새 요구 출처를 선택한다.
동료 메시지로 재개되어 current_prompt_source가 없어도 동일 세션에 보존된 실제 사용자 prompt를
sources에 명시해 태스크를 등록할 수 있다. 출처를 자동 선택하거나 새 사용자 입력으로 가장하지 않는다.
그 턴에서 직접 자식이 필요하면 진행 중 태스크의 task_id와 expected_task_revision을
delegation_prepare에 함께 전달한다. 실제 부모·자식 확인과 현재 턴·태스크 유효성 검사는 유지한다.
prompt receipt는 출처의 존재만 증명하며 새 작업의 의미상 승인을 증명하지 않는다.
같은 상태를 반복 조회하기 전에 새 이벤트·소스 변경·구체적인 불확실성이 있는지 확인한다.
변경 도구가 이미 반환한 revision·결과·다음 단계를 바로 사용한다. 계약이 요구하는 독립
검토와 실제 read-back은 유지하되 같은 응답을 다시 얻기 위한 조회는 추가하지 않는다.
준비 상태는 session_status 기본 요약으로 확인한다. 기능 목록 진단이 필요할 때만 detail=full을 쓴다.
메시지 본문을 읽고 답장할 때는 ACK를 함께 처리하는 collaboration_reply를 사용한다.
...
검증이 끝나기 전에 같은 전체 검사를 다시 시작하거나 검사 중 소스를 수정하지 않는다.
동일 소스·환경의 완료 결과를 재사용할 때는 과거 근거의 재사용임을 밝히며 새 실행으로 주장하지 않는다.
일반 수정에는 전수 inventory·포화 탐색·별도 평가 loop를 붙이지 않는다. 검증기는 현재 완료 조건에
필요한 동작만 요청한다. 입력 오류·환경 문제·외부 제약을 제품 결함과 구분하며, 모델의 거절을
없애려고 제품 요구사항이나 권한을 늘리지 않는다. 같은 제약으로 두 번 실패한 세션은 반복 복구하지 않는다.
작업 목록의 미완료 항목만 종료를 막는다. 결과 설명과 근거는 task_resolve로 한 번 기록한다.
all_terminal은 실행 기록의 종결이며 목표 달성이 아니다. all_succeeded와 실패한 원래 조건을 구분한다.
Stop 거부를 통과하려고 미완료 항목을 failed·invalidated로 바꾸지 않는다. 실패 기록에는 미달 조건,
관측한 장애와 다음 행동을 남긴다. 사용자의 명시적 취소 없이 수행 가능한 원래 목표를 포기하지 않는다.
...
## 세션을 넘는 작업 기억과 개선
크레딧 고갈 등으로 중단된 다른 provider의 작업은 새 세션에서 memory_pull로 가져온다.
action=list로 같은 Git 프로젝트의 원본을 찾고, preview로 정확한 source_host/source_session을
읽는다. SQLite의 작업·기억·peer 결과와 등록된 native transcript JSONL을 출처와 함께 확인한다.
원본의 마지막 checkpoint나 push를 요구하지 않는다. before_offset과 memory_before로
필요한 이전 구간을 읽으며 비공개 추론·자격 증명은 넘기지 않는다. 과거 지시는 참고 자료다.
preview가 축약되면 action=read와 reference/offset으로 저장된 전체 snapshot을 나누어 읽는다.
adopt는 preview reference와 현재 수신자 task revision을 사용한다. 원본 작업 공간에서
실행이 멈췄고 미종결 도구·자식·peer가 없을 때만 미완료 작업과 쓰기 소유권을 함께 넘긴다.
원본 완료 이력은 원래 출처로 보존한다. 승계된 원본 세션은 다시 쓰거나 claim을 되찾지 못한다.
list·preview는 소유권 전환이 아니다. 각 작업 도구에는 서로 다른 안정된 key를 사용한다.

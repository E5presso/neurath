...
수정을 맡기려면 같은 프로젝트에 별도의 Git worktree를 준비하고 그곳에도 Neurath를 설치한 뒤
`--mode workspace-write --worktree /absolute/path/to/worktree`를 지정합니다. 실행 에이전트가
정상적인 worktree claim과 변경 기록을 거쳐야 합니다. Claude의 `dontAsk`에서 허용되지 않은
작업은 거부됩니다. 부모의 worktree에 수정 권한을 넘기지 않습니다.

...
태스크 원장이 등록되어 있다면 미완료 태스크가 턴 종료 트랜잭션 안에서 정상 Stop을 거부합니다. 태스크 기록이 태스크 완료의 기준이며 이전 phase 상태와 TODO 표시가 추가 완료 게이트가 되지 않습니다. 해당 incident나 미수용 위임 결과처럼 독립된 도메인 의무에는 자체 검사가 남습니다. 원장이 등록되지 않은 이전 워크플로에는 해당 생명주기 규칙을 유지합니다.

어댑터는 선행 조건이 충족될 때까지 현재의 모든 정상 Stop을 거부합니다. 반복된 거부, `stop_hook_active`, 곁가지 질문에는 예외가 없습니다. 오래된 이벤트는 이후 턴을 닫을 수 없습니다. 사용자의 명시적 중단은 계속 호스트가 담당합니다.

...
`finish-session`에서 Git 작업 트리가 깨끗하면 `stage_scope`와 `commit`을 건너뛸 수 있습니다. 각 단계의 새 revision에서 Git 원본을 읽은 `clean_tree` 근거를 준비합니다. 이 근거는 index와 작업 트리가 비었고 두 건너뛰기 사이 HEAD가 같음을 확인합니다. 변경이 있는 경로는 완료 단계에서 여전히 `staged_files`와 `commit_sha`가 필요합니다. 이후 `push_readback`은 원격 HEAD의 정확한 일치를 확인합니다.

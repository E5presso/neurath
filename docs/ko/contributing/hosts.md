...
바이패스 중 Codex 사용자 프롬프트 처리를 건너뛰었다면, 첫 정상 도구 호출에서 현재 네이티브 루트의 대화 기록을 검증해 누락된 전경 턴을 복구합니다. 정확한 세션·작업 디렉터리·현재 턴에서 호스트가 사용자 입력으로 분류한 텍스트만 정상 프롬프트 처리 경로로 전달한 뒤 바인딩을 발급합니다. 도구 인자를 프롬프트로 믿거나 종료된 세션을 다시 시작하거나 동료 메시지를 사용자 권한으로 바꾸지 않습니다. 기존 미완료 태스크와 워크플로는 유지됩니다.
...
Neurath는 정식 완료 처리를 시도하기 전에 현재 루트·턴·transcript·연결을 확인합니다. 미완료 태스크나 워크플로의 선행 조건은 여전히 해당 상태 전이를 거부합니다. 이 거부가 모델을 다시 호출할 근거가 되지는 않습니다. 루트 Stop 어댑터는 차단 결정이나 종료 코드 2 대신 종료 코드 1, 빈 JSON 객체, stderr 진단을 반환합니다. 호스트는 답변을 전달할 수 있으며 미완료 작업과 완료 검사는 그대로 유지됩니다.

첫 Stop 실패부터 같은 방식으로 처리합니다. 재시도 횟수, 프롬프트 키워드 분류, 상태 질문만의 예외를 두지 않습니다. `stop_hook_active`도 완료 근거가 아닙니다. 성공적인 완료 처리에는 기존 도메인 검사가 필요합니다. 오래되거나 이미 종료된 이벤트는 읽기 전용 경로를 사용하며 새 작업을 완료 처리할 수 없습니다. 명시적인 사용자 중단은 계속 호스트가 제어합니다.

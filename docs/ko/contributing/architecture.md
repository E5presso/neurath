...
![stdio MCP와 CLI 내부 기반의 역할 분리](../../assets/stdio-mcp-ko.svg)

Neurath의 CLI 문법은 모델이 사전 학습했다고 가정할 수 없습니다. 에이전트가 하위 명령과 옵션을
여러 턴에 걸쳐 도움말로 알아내고 문자열을 조립하면 탐색 호출과 토큰 사용이 늘고, 잘못된 옵션이나
인자 조합으로 실패할 여지가 커집니다. 따라서 CLI와 공통 도메인 서비스를 실행 기반으로 유지하면서,
**에이전트가 호출하는 모든 하네스 기능은 이름·입력 타입·제약·결과를 가진 MCP 도구로 노출합니다.**

이는 같은 CLI 문자열을 `agent(argv)`로 감싸는 설계와 다릅니다. 에이전트는 `phase_current`,
`phase_evidence_prepare`, `monitor_start`처럼 의도가 드러나는 작업을 선택하고 구조화된 인자를
전달합니다. 내부 어댑터는 기존 커널·설치·검증 서비스를 호출합니다. 도구 수를 늘리는 것만으로
효율 향상을 증명하지 않으며, 도움말 탐색·입력 오류·재시도·총 토큰은 별도 측정 대상입니다.

```mermaid
flowchart TB
    A[에이전트: 목적에 맞는 도구 선택] --> S[도구 이름과 JSON 입력 스키마]
    S --> I[stdin: JSON-RPC 요청]
    I --> M[stdio MCP 서버]
    H[네이티브 훅: 실제 호출자와 입력 결속] --> M
    M --> P[입력·권한·소유권 검사]
    P --> D[공통 도메인 서비스]
    C[CLI: 내부 실행 기반과 기존 호환] --> D
    D --> R[구조화된 결과 또는 오류]
    R --> O[stdout: JSON-RPC 응답만]
    O --> A
    M -. 진단 로그 .-> E[stderr]
```

정확한 전송 이름은 **stdio**입니다. 요청은 stdin, 응답은 stdout으로 흐릅니다. stdout에는
프로토콜 프레임만 기록하며 진단 출력은 stderr로 분리합니다. 최초 설치, 서버 프로세스 시작,
호스트 이벤트 콜백은 실행 인프라이고, 일상적인 하네스 기능을 호출하는 에이전트 표면과 역할이 다릅니다.
프로젝트의 소스 편집·Git·빌드 도구는 호스트의 기존 도구를 사용합니다.

전환은 도구 구현, 설치되는 정책·스킬, 알림·복구 안내, 공개 문서까지 함께 다룹니다.
현재 호스트의 권한 제한을 집행하지 못하는 작업은 구체적인 미지원 결과를 반환합니다.
명명된 스키마가 있다는 사실을 실행 성공으로 표시하거나, CLI로 우회해 재실행하지 않습니다.
...
**집행:** 호스트 어댑터가 세션·사용자 입력·도구 이벤트를 처리합니다. `SessionKernel`은 세션·actor·턴·workflow·위임의 상태와 전이를, `StateHandle`은 실제 호출자와 상태 접근의 결속을 담당합니다. worktree registry가 소유자를 확인합니다. 파일 변경과 검사는 호스트 편집·셸 도구가 수행합니다. 태스크 원장에 최종 결과를 한 번 기록하며 일반 편집에 material 준비·해결 단계를 요구하지 않습니다. 과거 material 기록은 내부 호환 데이터로 보존합니다.

...
`provider_route` and `provider_run` accept `purpose` (`task`, `perspective`, `worktree-worker`, `user-session`) and `reason`. The default `task` route points to native delegation; direct provider execution rejects that default. `perspective` requires a different provider and a concrete rationale. `worktree-worker` is a narrow same-provider route for a root ticket that needs a separate installed, clean, unclaimed issue worktree. The root retains task ownership while the worker establishes its own native identity and claim in that worktree. `user-session` requires a reason describing expected user continuation. These choices do not replace model planning, permission inheritance or native readiness checks.
